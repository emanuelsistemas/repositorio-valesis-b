...
import { FolderPlus, LogOut, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="w-96 bg-gray-800 p-6 overflow-y-auto border-r border-gray-700">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-bold">Repositório</h1>
        <button
          onClick={onLogout}
      ...
      {!isConnected && (
        <div className="mb-4 p-3 bg-yellow-600 text-white rounded-md flex items-center text-sm">
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-500 text-white rounded-md flex items-center text-sm">
          <AlertCircle className="w-4 h-4 mr-2" />
          <span>{error}</span>
        </div>
      )}
